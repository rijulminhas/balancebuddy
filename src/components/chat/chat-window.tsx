...
export function ChatWindow({
  ...
  const { connected, onlineUsers, typingUsers, readReceipts, sendMessage, deleteMessage, reactMessage, sendTyping, markRead } = useSocketChat({
    onNewMessage: (msg) => {
      setMsgList((prev) => {
  ...
  // Mark the latest message as read whenever the chat updates while the tab is visible.
  const latestCreatedAt = msgList.length ? msgList[msgList.length - 1].createdAt : null;
  useEffect(() => {
    if (!connected || !latestCreatedAt) return;
    if (typeof document !== "undefined" && document.visibilityState === "hidden") return;
    markRead(latestCreatedAt);
  }, [connected, latestCreatedAt, markRead]);

  // Also mark read when the tab regains visibility.
  useEffect(() => {
    function onVisible() {
      if (document.visibilityState === "visible" && latestCreatedAt) markRead(latestCreatedAt);
    }
    document.addEventListener("visibilitychange", onVisible);
    return () => document.removeEventListener("visibilitychange", onVisible);
  }, [latestCreatedAt, markRead]);

  // The current user's most recent text/image message — the one that shows "Seen by".
  const lastOwnMessageId = (() => {
    for (let i = msgList.length - 1; i >= 0; i--) {
      const m = msgList[i];
      if (m.senderId === currentUserId && (m.type === "text" || m.type === "image")) return m.id;
    }
    return null;
  })();

  const lastOwnCreatedAt = lastOwnMessageId
    ? msgList.find((m) => m.id === lastOwnMessageId)?.createdAt ?? null
    : null;

  const seenBy = lastOwnCreatedAt
    ? readReceipts.filter(
        (r) =>
          r.userId !== currentUserId &&
          new Date(r.lastReadAt).getTime() >= new Date(lastOwnCreatedAt).getTime(),
      )
    : [];
          ...
          {msgList.length === 0 ? (
            ...
            msgList.map((msg) => (
              <MessageBubble
                ...
                onReply={handleReply}
                onReact={handleReact}
                seenBy={msg.id === lastOwnMessageId ? seenBy : undefined}
              />
            ))
          )}
          <div ref={bottomRef} />
