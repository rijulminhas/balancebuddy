  ...
  function getSplitError(): string | null {
    ...
    if (splitType === "percentage") {
      const total = selectedMembers.reduce(
        (s, id) => s + (parseFloat(customSplits[id] ?? "0") || 0), 0
      );
      if (Math.abs(total - 100) > 0.01)
        return `Percentages must sum to 100% (currently ${fmt(total)}%)`;
    }
    if (splitType === "amount") {
      const total = selectedMembers.reduce(
        (s, id) => s + (parseFloat(customSplits[id] ?? "0") || 0), 0
      ...
      if (Math.abs(total - amount) > 0.01)
        return `Amounts must sum to ₹${fmt(amount)} (currently ₹${fmt(total)})`;
    }
    return null;
  }
