...
interface RemoveMemberModalProps {
  groupId: string;
  requesterId: string;
  target: { userId: string; name: string; isProtected?: boolean };
  open: boolean;
    ...
    return (
      <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Can&apos;t remove {target.name}</DialogTitle>
            <DialogDescription>
              {target.name} is a protected super admin account and cannot be
