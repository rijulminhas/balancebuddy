...
export async function removeMember(
  ...
  const [targetMember] = await db
    .select({ id: groupMembers.id, role: groupMembers.role, email: users.email })
    .from(groupMembers)
  ...
  if (isSuperAdmin(targetMember.email)) {
    return { success: false, error: "This member cannot be removed from the group" };
  }
...
export async function switchGroup(
  ...
  const [alreadyInTarget] = await db
    .select({ id: groupMembers.id, status: groupMembers.status })
    .from(groupMembers)
    .where(and(eq(groupMembers.groupId, targetGroup.id), eq(groupMembers.userId, userId), eq(groupMembers.status, "active")))
    .limit(1);
  ...
  const [currentMembership] = await db
    .select({ id: groupMembers.id, groupId: groupMembers.groupId, role: groupMembers.role, joinedAt: groupMembers.joinedAt })
    .from(groupMembers)
    .where(and(eq(groupMembers.userId, userId), eq(groupMembers.status, "active")))
    .limit(1);

  if (currentMembership) {
    // Owner cannot switch without transferring ownership first
    if (currentMembership.role === "owner") {
      const [otherActiveMember] = await db
        .select({ id: groupMembers.id })
        .from(groupMembers)
        .where(and(eq(groupMembers.groupId, currentMembership.groupId), eq(groupMembers.status, "active"), ne(groupMembers.userId, userId)))
        .limit(1);

      if (otherActiveMember) {
        return { success: false, error: "You are the group owner. Transfer ownership before switching groups." };
      }
    }

    const [currentGroupData] = await db
      .select({ name: groups.name, address: groups.address, inviteCode: groups.inviteCode })
      .from(groups)
      .where(eq(groups.id, currentMembership.groupId))
      .limit(1);

    await db
      .update(groupMembers)
      .set({ status: "left", updatedAt: new Date() })
      .where(eq(groupMembers.id, currentMembership.id));

    const now = new Date();
    const [existingHistory] = await db
      .select({ id: groupHistory.id, joinCount: groupHistory.joinCount })
      .from(groupHistory)
      .where(and(eq(groupHistory.userId, userId), eq(groupHistory.groupId, currentMembership.groupId)))
      .limit(1);

    if (existingHistory) {
      await db
        .update(groupHistory)
        .set({ groupName: currentGroupData.name, groupAddress: currentGroupData.address, role: currentMembership.role, lastLeftAt: now, joinCount: (existingHistory.joinCount ?? 1) + 1, inviteCode: currentGroupData.inviteCode, deletedAt: null, deletedByOwner: false, updatedAt: now })
        .where(eq(groupHistory.id, existingHistory.id));
    } else {
      await db.insert(groupHistory).values({ userId, groupId: currentMembership.groupId, groupName: currentGroupData.name, groupAddress: currentGroupData.address, role: currentMembership.role, firstJoinedAt: currentMembership.joinedAt ?? now, lastLeftAt: now, joinCount: 1, inviteCode: currentGroupData.inviteCode });
    }
  }
  ...
  if (existingTargetMember) {
    await db.update(groupMembers).set({ status: "active", joinedAt: new Date(), updatedAt: new Date() }).where(eq(groupMembers.id, existingTargetMember.id));
  } else {
    await db.insert(groupMembers).values({ groupId: targetGroup.id, userId, role: "member", status: "active", joinedAt: new Date() });
  }

  const [joiningUser] = await db.select({ name: users.name }).from(users).where(eq(users.id, userId)).limit(1);

  const activeMembers = await db
    .select({ userId: groupMembers.userId })
    .from(groupMembers)
    .where(and(eq(groupMembers.groupId, targetGroup.id), eq(groupMembers.status, "active")));

  const notifyIds = activeMembers.map((m) => m.userId).filter((id) => id !== userId);
  if (notifyIds.length > 0) {
    await notifyUsers(notifyIds, targetGroup.id, "member_joined", `${joiningUser?.name ?? "Someone"} joined the group`, `${joiningUser?.name ?? "A new member"} has joined ${targetGroup.name}.`, { url: "/groups" });
  }
  ...
  return { success: true, data: { groupId: targetGroup.id } };
}
